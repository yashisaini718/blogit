...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Account created! You can now login with your credentials.', 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            return redirect(next_page) if next_page else redirect(url_for('main.home')) #ternary operator used 
        else:
            flash('Unsuccessful Login. Check your email and password!','danger')
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password has been updated. Now you can login with your new password!', 'success')
        return redirect(url_for('users.login'))
